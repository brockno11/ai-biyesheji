...
| 覆盖范围 | 说明 |
|----------|------|
| 线性回归 | 模型系数、预测长度、R²、MSE 自动验证 |
| KNN | 分类器类别、预测长度、accuracy 自动验证 |
| 决策树 | 树结构、预测长度、accuracy、树深度自动验证 |
| K-Means | 聚类中心、标签数、inertia 自动验证 |
| 执行方式 | Web Worker 中加载 Pyodide，执行学生 Python 代码（不阻塞 UI） |
...
| 错误分类 | 自动归类语法错误/变量未定义/依赖缺失/维度错误/模型未训练/超时 |
| 兜底策略 | 非支持题目显示”暂未开启真运行”，继续使用规则检查 + AI 诊断 |
...
| 不足 | 说明 | 后续优化方向 |
|------|------|-------------|
| **无用户系统** | 当前仍是单用户 localStorage 模式 | 后续加入登录、数据库、云端同步 |
| **无教师端** | 当前主要面向学生自学和答辩演示 | 后续增加班级管理、作业布置、学生统计 |
...
| **v2.8.0** | 基础课微课化：14 个 lesson 小节 + 二级 Sidebar 学习路径导航 + 5 个交互任务组件 + lesson 进度存储 |
