...
│   ├── ProfilePage.tsx         # 个人中心 (学习概览+管理员入口)
...
| `/progress` | ProgressPage | Layout (Header+Sidebar) | 学习进度 |
...
| 区域 | 内容 |
|------|------|
| 顶部导航 | Logo + 课程链接 + 学习进度 + 个人中心入口 + "开始学习"按钮 |
...
**功能定位**：学习者的个人数据面板 + 管理员入口。

**页面布局**：顶部 Header + 个人信息卡片 + 学习数据区

**组成部分**：

| 模块 | 内容 |
|------|------|
| 个人信息卡 | 头像 + 昵称 + 学习状态 + **管理员入口按钮** |
| 统计卡片 | 已完成算法 / 练习次数 / 最高得分 / 测验次数 |
| 总体进度 | 进度条 |
| 各算法详情 | 状态 + 练习/得分/测验 + 跳转 |
| 最近活动 | 最后一次练习记录 |
| 重置按钮 | 清除进度 |

**管理员入口**：个人信息卡片右侧的 🛡️ 按钮，点击进入 `/admin`

**操作逻辑**：
1. 用户从首页 ⚙️ 或侧边栏"个人中心"→ 进入此页
2. 查看学习数据
3. 点击"管理员入口"→ 进入管理后台
4. 或点击各算法"进入"→ 跳转到课程页
...
| **完整的演示闭环** | 首页→课程→可视化→练习→测验→进度，所有页面可完整走通 |
...
| **UI 设计规范** | 统一的蓝紫渐变 + 16px 圆角 + 白色卡片设计语言 |
| **响应式布局** | 首页和算法页做了响应式适配 |
...
| **无移动端深度适配** | 部分页面在小屏幕上体验不佳 | 全面响应式改造 |
