...
export default function AdminPage() {
  ...
  const [adminLoginUser, setAdminLoginUser] = useState('admin');
  const [adminLoginPass, setAdminLoginPass] = useState('123456');
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 min-w-0">
            {/* Backend admin login gate — shown globally before any tab */}
            {!adminLoggedIn && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-lg font-bold text-gray-900 mb-1">管理员后端登录</h2>
                  <p className="text-sm text-gray-500">登录后端管理服务以管理课程、题库和查看操作日志</p>
                </div>
                <div className="bg-white/70 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm p-6">
                  <div className="flex items-center gap-2 mb-4">
                    <Server className="w-5 h-5 text-amber-600" />
                    <span className="text-sm font-bold text-gray-900">后端管理服务</span>
                    {backendOnline === null ? (
                  ...
                  {backendOnline && (
                    <form onSubmit={handleAdminLogin} className="flex flex-wrap gap-3 items-end">
                      <div>
                        ...
                        <input value={adminLoginUser} onChange={(e) => setAdminLoginUser(e.target.value)} className="px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 bg-gray-50/50 w-36" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">密码</label>
                        <input type="password" value={adminLoginPass} onChange={(e) => setAdminLoginPass(e.target.value)} className="px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 bg-gray-50/50 w-36" />
                      </div>
                  ...
                  <p className="mt-3 text-xs text-gray-400">默认管理员账号：admin / 123456（可在 .env 中修改 ADMIN_USERNAME / ADMIN_PASSWORD）</p>
                </div>
              </div>
            )}

            {adminLoggedIn && activeTab === 'dashboard' ? (
              /* ═══ Dashboard ═══ */
              <div className="space-y-6">
                <div>
            ...
            ) : adminLoggedIn && activeTab === 'courses' ? (
              /* ═══ Course Management ═══ */
              <AdminCoursePanel />
            ) : adminLoggedIn && activeTab === 'audit' ? (
              /* ═══ Audit Logs ═══ */
