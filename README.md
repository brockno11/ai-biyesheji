# 基于 AI 赋能的机器学习算法教学平台

> **文档导航**：快速了解项目 → 当前页；深入技术细节 → **[DOCS.md](./DOCS.md)**；AI Agent 开发 → **[SKILLS.md](./SKILLS.md)**
>
> **当前版本：v2.8.1** | 最近更新：9 门课程（3基础+6算法）、引导思考题、Python 代码入门桥梁课、学习路径按难度重排

## 项目简介

这是一个面向机器学习初学者的交互式 Web 教学平台。项目通过 **课程学习、算法可视化、在线代码练习、知识测验、AI 助教和学习中心** 组成完整学习闭环，适合毕业设计答辩、课程设计展示和个人自学演示。

当前界面采用清爽的学习工具风格：顶部导航保留首页、学习中心、个人中心；课程列表集中在内页侧边栏；管理后台入口只放在个人中心，避免普通学习路径被打断。

## 技术栈

| 技术 | 用途 |
|------|------|
| React 18 | UI 框架 |
| TypeScript | 类型安全 |
| Vite 6 | 构建工具 |
| Tailwind CSS 3 | 样式方案 |
| React Router v6 | 前端路由 |
| Monaco Editor | 在线代码编辑器 |
| Pyodide | 浏览器端执行 Python 代码 |
| ECharts / echarts-for-react | 算法可视化 |
| Lucide React | 图标 |
| localStorage | 学习进度和自定义课程本地持久化 |
| Node / Express | DeepSeek API 后端代理 |
| DeepSeek OpenAI-compatible API | 真实 AI 助教；默认 Mock 兜底 |

## 功能模块

### 当前项目状态（2026-05-09）

- 内置课程：9 门（3 门基础概念课 + 6 门算法课），面向零基础渐进式学习，按入门→中级难度编排
- 代码练习：8 道（覆盖线性回归、KNN、决策树、K-Means 四门算法；逻辑回归和随机森林练习待后续开放）
- 测验题目：72 道（覆盖全部 9 门课程，含基础课 24 道 + 算法课 48 道）
- 可视化组件：6 个（LinearRegressionViz、KNNViz、DecisionTreeViz、KMeansViz、LogisticRegressionViz、RandomForestViz）
- AI 模块：DeepSeek 在线模式 + Mock 离线兜底，覆盖 AI 助教、代码诊断、错题讲解、学习路径推荐、课程草稿生成和可视化解释
- 管理后台：课程 CRUD + 题库/练习 CRUD，本地数据通过 localStorage 持久化

### 1. 首页

- 展示平台定位和核心能力
- 主按钮“开始学习”直接进入第一门算法课程
- 次级入口“查看学习进度”进入学习中心
- 展示算法数量、练习题、测验题和 AI 助教等概览指标

### 2. 学习中心

- 展示总体学习进度
- 展示算法完成数、练习次数、最高得分、测验次数
- 展示每门算法的学习状态、练习次数、最佳得分和测验成绩
- 推荐下一步学习内容
- 新增 AI 学习路径推荐，结合本地学习记录生成今日计划

### 3. 课程学习

- 内置 9 门课程：3 门基础概念课（机器学习入门、数据与评估、Python 代码入门）+ 6 门算法课（线性回归、K近邻、逻辑回归、决策树、K-Means、随机森林），按入门→中级难度编排
- 每门课程包含简介、公式、步骤、优缺点、适用场景、Python 示例代码
- 支持 B 站教学视频嵌入
- 支持交互式算法可视化

### 4. 算法可视化

- **线性回归**：散点图、拟合直线、Loss 曲线，支持学习率、迭代次数、噪声等参数调节
- **KNN**：二维分类散点图、K 值调节、最近邻居高亮、测试点点击移动
- **决策树**：树结构可视化、最大深度调节、样本分类路径追踪
- **K-Means**：聚类散点图、K 值调节、迭代次数、簇内分散度、聚类中心与 inertia 指标
- **逻辑回归**：2D 分类散点 + Sigmoid 曲线 + 阈值滑块 + 决策边界
- **随机森林**：多棵决策树卡片 + 投票汇总 + n_estimators/max_depth 滑块 + 特征重要性柱状图
- **AI 洞察**：每个可视化组件支持点击"AI 解释当前现象"，AI 根据当前参数解释模型表现

### 5. 代码练习

- Monaco Editor 在线 Python 编辑器
- 基于规则的代码检查 + Pyodide 浏览器端 Python 真运行
- 每道练习通过 Web Worker 执行 Pyodide 真运行，覆盖线性回归/KNN/决策树/K-Means
- 综合评分按核心 API、TODO 完成度、Python 可执行性、固定测试表现四个维度计算
- 实时显示需要使用的 API
- 保存练习记录、得分和反馈
- 检查后自动生成结构化 AI 代码诊断报告，AI 会结合本地规则和 Python 报错给出修改方向

### 6. 知识测验

- 每门课程配套选择题测验，当前共 72 道内置测验题（含基础课 24 道）
- 自动评分
- 提交后展示正确答案和解析
- 测验记录保存到本地
- 提交后可触发 AI 错题讲解和相似练习题

### 7. AI 助教

- 支持 DeepSeek 在线模式和 Mock 离线演示模式，自动 fallback
- 支持概念解释、代码诊断（结构化报告卡片）、学习建议、题目生成、错题讲解（薄弱点分析+相似题）、学习路径推荐（今日计划）、课程草稿生成
- 可视化组件内嵌 AI 洞察：解释参数变化对模型的影响
- 全局 AI 模式徽标，用户始终知道当前是 AI 还是 Mock 在回答

### 8. 个人中心

- 学习仪表盘：完成度圆环、关键学习指标、课程掌握情况、最近活动
- 推荐下一步课程
- 提供“课程管理入口”
- 支持重置学习进度

### 9. 管理后台

- 从个人中心进入，不出现在顶部导航栏
- 查看系统概览
- 管理自定义课程
- 支持添加、编辑、删除、搜索和预览课程
- 支持题库与练习管理：新增/编辑/删除自定义测验题和练习题，也可以对内置题目做本地覆盖编辑

## 安装和运行

```bash
# 1. 进入项目目录
cd E:\projects\ai-biyesheji

# 2. 安装依赖
npm install

# 3. 启动开发服务器和 AI 后端代理
npm run dev
```

默认访问地址：

```text
前端：http://localhost:3000
AI 代理：http://localhost:8787
```

`npm run dev` 会同时启动 Vite 和 Express 代理。浏览器只请求 `/api/ai/chat`，真实 DeepSeek API Key 只在 Node 后端读取，不会打进前端包。

启动和构建前会自动执行 `npm run sync:pyodide`，将 Pyodide 核心运行文件同步到 `public/pyodide`。Python 包（numpy / scikit-learn）由 Pyodide 按需获取，加载失败时页面会展示错误并交给 AI 诊断解释。

可用 `http://localhost:8787/api/ai/status` 查看 AI 代理状态；该接口只返回是否已配置 Key，不返回真实密钥。

## 生产构建

```bash
npm run build
npm run preview
```

## DeepSeek API 配置

不设置环境变量时，所有 AI 功能自动走 Mock 离线演示模式，不影响完整演示。

```env
# 在项目根目录创建 .env 文件
DEEPSEEK_API_KEY=your-api-key
DEEPSEEK_BASE_URL=https://api.deepseek.com
DEEPSEEK_MODEL=deepseek-v4-flash
AI_ENABLE_MOCK_FALLBACK=true
```

> 安全注意：不要把 API Key 写入前端代码，不要提交 `.env`。详细配置说明和环境变量含义见 **[DOCS.md 第十二章](./DOCS.md#十二deepseek-ai-模块升级说明)**。

## AI 答辩演示流程

覆盖课程学习、代码练习、测验、学习中心、管理后台五大场景的完整演示路径。

> 分步演示细节见 **[DOCS.md 第十二章 → 答辩演示步骤](./DOCS.md#126-答辩演示步骤)**。

## 项目亮点

1. **零数据库依赖**：静态课程/题库数据 + localStorage 自定义覆盖；AI 部分采用轻量 Node/Express 代理。
2. **学习闭环完整**：课程学习 → 可视化探索 → 代码练习 → 测验验证 → 学习中心追踪。
3. **AI 可在线也可离线**：DeepSeek 在线模式 + Mock fallback 覆盖核心助教能力。
4. **代码练习更可信**：规则检查 → Pyodide 真运行 → 测试结果 → AI 诊断，能展示代码是否真的跑通。
5. **学习中心清晰**：用户可以直接查看进度、下一步推荐和每门课程掌握情况。
6. **管理入口收敛**：管理后台只从个人中心进入，主导航保持学习路径简洁。
7. **内容更完整**：9 门课程、8 道代码练习、72 道测验题、6 个交互式可视化组件，管理后台还能继续扩展题库。
8. **答辩友好**：界面完整、路径清楚、功能可现场走通。

## 设计原则

- **学习优先**：顶部导航围绕”首页、学习中心、个人中心”组织。
- **操作清晰**：首页强化”开始学习”，学习中心承担进度查看。
- **演示稳定**：没有真实 API Key 也能展示 AI 助教。
- **本地持久化**：学习记录和自定义课程保存在浏览器 localStorage。
- **易扩展**：课程数据、练习数据、测验数据和 UI 组件分层清晰。

## AI Agent 开发指引

本项目配置了 19 个 Claude Code Skills（位于 `.claude/skills/`），覆盖前端开发、数据可视化、后端 API、代码审查、测试、UI 设计、动画动效、教育注释、学术写作、文档处理等场景。

> 新 Agent 加入开发前，请先阅读 **[SKILLS.md](./SKILLS.md)** 了解全部技能清单、安装方式和本地路径。

## 后续扩展方向

短期：E2E 测试、综合测验、Pyodide 加载优化；中期：用户系统、新增算法（SVM/神经网络/朴素贝叶斯）、教师端；长期：社区、移动端 App、国际化。

> 完整规划（短期/中期/长期）见 **[DOCS.md 第九章](./DOCS.md#九后续可扩展方向)**。
