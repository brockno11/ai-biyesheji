...
>
> **当前版本：v2.8.2** | 最近更新：课程2升级10小节+10互动组件+评估调参模块+概念掌握度+管理员双表单
...
- **交互式可视化：6 个**（LinearRegressionViz、KNNViz、DecisionTreeViz、KMeansViz、LogisticRegressionViz、RandomForestViz）
- **教学图解：15 个**（Diagram 组件，覆盖 AI/ML/DL 嵌套、任务对比、ML 流程、数据拆分、过拟合等）
- **互动实验组件：10 个**（回归指标/分类指标/过拟合/超参数/CV/数据泄露侦探等，14/14 互动类型全部实现）
- **AI 助教：11 种场景**（问答/概念解释/代码诊断/错题讲解/学习计划/课程草稿/出题/总结/生活例子/可视化解释/练习提示）
...
- 11 种 AI 场景，JSON 模式输出经 Zod 校验
- API Key 仅在后端环境变量，前端通过 `/api/ai/chat` 代理调用

### 8. 管理后台

- 个人中心 → 课程管理入口（不出现在主导航栏）
- 课程管理：foundation/algorithm 类型感知双表单 + AI 课程草稿生成
- 题库管理：练习题/测验题 CRUD + 启用/停用 + AI 生成 + 导入/导出
...
3. **AI 可在线也可离线**：DeepSeek + Mock fallback 覆盖 11 种助教能力
4. **代码练习全 Pyodide 覆盖**：12/12 道练习均含真运行+固定测试
5. **互动全覆盖**：14/14 InteractionType 均实现专属组件或问答兜底
6. **管理入口收敛**：管理后台仅从个人中心进入
7. **内容完整**：9 门课程、12 道练习(全Pyodide)、78 道测验、15 图解+10 互动实验+20+ 图解
